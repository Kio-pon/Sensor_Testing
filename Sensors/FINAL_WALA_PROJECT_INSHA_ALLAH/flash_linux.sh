...
# Add STM32 GNU Tools path (arm-none-eabi compiler)
STM32_GNU_TOOLS="${HOME}/.local/share/stm32cube/bundles/gnu-tools-for-stm32/14.3.1+st.2/bin"
if [[ -d "$STM32_GNU_TOOLS" ]]; then
    export PATH="${STM32_GNU_TOOLS}:${PATH}"
    info "Added STM32 GNU Tools to PATH"
fi

# Add STM32CubeIDE toolchain paths if available
STM32_IDE_CORE="${HOME}/.vscode/extensions/stmicroelectronics.stm32cube-ide-core-1.3.0-linux-x64/resources/binaries/linux/x86_64"
STM32_IDE_CMAKE="${HOME}/.vscode/extensions/stmicroelectronics.stm32cube-ide-build-cmake-1.45.0-linux-x64/resources/cube-cmake/linux/x86_64"

if [[ -d "$STM32_IDE_CORE" ]]; then
    export PATH="${STM32_IDE_CORE}:${PATH}"
    info "Added STM32CubeIDE toolchain to PATH"
fi

if [[ -d "$STM32_IDE_CMAKE" ]]; then
    export PATH="${STM32_IDE_CMAKE}:${PATH}"
    info "Added CMake tools to PATH"
fi

# Check for cube tool (STM32CubeIDE wrapper)
if ! command -v cube &>/dev/null; then
    if command -v arm-none-eabi-gcc &>/dev/null; then
        info "Using system ARM toolchain"
    else
        die "Neither STM32CubeIDE toolchain nor system ARM toolchain found"
    fi
fi

for tool in st-flash; do
    command -v "$tool" &>/dev/null || MISSING_TOOLS+=("$tool")
done

# Check for CMake or cube-cmake
if ! command -v cmake &>/dev/null && ! command -v cube-cmake &>/dev/null; then
    MISSING_TOOLS+=("cmake/cube-cmake")
fi
...
GCC_VERSION=$(arm-none-eabi-gcc --version 2>/dev/null | head -1 || echo "STM32CubeIDE Toolchain")
...
# Build dir is per-type so all configs can coexist side-by-side
...
CMAKE_CMD="cmake"
CUBE_CMAKE="${HOME}/.vscode/extensions/stmicroelectronics.stm32cube-ide-build-cmake-1.45.0-linux-x64/resources/cube-cmake/linux/x86_64/cube-cmake"

if [[ -f "$CUBE_CMAKE" ]]; then
    CMAKE_CMD="$CUBE_CMAKE"
    info "Using STM32CubeIDE CMake wrapper"
else
...
# Use cube wrapper or cmake
if [[ -f "$CUBE_CMAKE" ]]; then
    "$CUBE_CMAKE" --build "$BUILD_DIR" -j"$JOBS" || die "Build failed"
else
